...
public class StartNewProcess {
    ...
    public static void main(String[] args) {
        ...
        for (long sleptFor = 0; sleptFor < sleepDuration; sleptFor += sleepInterval) {
            startProcess(sleepInterval, sleepDuration);
            try {
    ...
    public static void startProcess(long sleepInterval, long sleepDuration) {
        Process process = newProcess(sleepInterval, sleepDuration);
        ProcessHandle handle = process.toHandle();
        CompletableFuture<ProcessHandle> future = handle.onExit();
        // Print a message when process terminates
        future.thenAccept((ProcessHandle ph) -> System.out.printf("Job (pid=%d) terminated.%n", ph.pid()));
    }
     ...
     */
    public static Process newProcess(long sleepInterval, long sleepDuration) {
     ...
     */
    private static long parseArg(String valueStr, long defaultValue) {
    ...
    private static void addMainClass(List<String> cmd) {
        Class<JobProcess> cls = JobProcess.class;
